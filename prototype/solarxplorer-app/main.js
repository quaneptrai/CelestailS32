...
setTimeout(() => {
  animate();
  // Live NASA/JPL calls are opt-in in the MCU learning build. The original
  // generated belts, orbit controls and visual UI remain unchanged.
  if (window.MCU_ENABLE_LIVE_NASA === true) initializeRealObjects();
}, 200);
...
// Music support removed from the MCU learning build.
...
if (planetList) {
  ...
  Object.entries(groupedBodies).forEach(([type, bodies]) => {
    ...
    bodies.forEach((body, localIndex) => {
      ...
      planetItem.dataset.bodyName = body.name;
      ...
      planetItem.addEventListener('click', (event) => {
        event.stopPropagation();
...
function showPlanetInfoCard(body, planetIndex) {
  ...
  if (body.moons && body.moons.length > 0) {
    ...
    // The ARIS learning renderer can replace the original moon-count span.
    // Keep planet selection working even after a topic library has occupied this panel.
    if (moonCount) moonCount.textContent = body.moons.length;
  ...
  window.dispatchEvent(new CustomEvent('solarxplorer:body-selected', {
    detail: { name: body.name, planetIndex }
  }));
}
...
function findNearbyPlanet(event) {
  const focalLength = window.innerHeight / (2 * Math.tan(THREE.MathUtils.degToRad(camera.fov * 0.5)));
  let nearest = null;
  ...
  planetMeshes.forEach((planetObj, planetIndex) => {
    ...
    const visualRadius = (celestialBodies[planetIndex].size / Math.max(distanceToCamera, 0.001)) * focalLength;
    const hitRadius = Math.max(34, Math.min(72, visualRadius * 1.65));
    const pointerDistance = Math.hypot(event.clientX - screenX, event.clientY - screenY);

    if (pointerDistance <= hitRadius && (!nearest || pointerDistance < nearest.pointerDistance)) {
      nearest = { planetIndex, pointerDistance };
    }
  });

  return nearest?.planetIndex ?? -1;
}
...
document.addEventListener('DOMContentLoaded', () => {
  const originalPlanetItemHandler = planetItem => {
    ...
    planetItem.onclick = function(event) {
      ...
      const planetName = this.dataset.bodyName || this.querySelector('strong').textContent;
